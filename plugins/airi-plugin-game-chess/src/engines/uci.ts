/**
 * UCI protocol helpers shared by both engines.
 *
 * Stockfish (chess) and Fairy-Stockfish (xiangqi) both speak UCI, so the search
 * driver and line parsing live here once and the engine adapters only differ in
 * how they transport text to/from the WASM module.
 */
 ...
 * `isready` → `readyok`. Engine-specific options (variant, thread count) are
...
export function analyzeViaUci(
  ...
  const latestByRank = new Map<number, RawCandidate>()
